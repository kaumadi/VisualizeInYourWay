...
//        }
    ...
    function getEpmsPerPrivileges($priv_code) {
        ...
        $perm = Access_controllerservice :: checkAccess('MANAGE_USER_PRIVILEDGES');
        if ($perm) {
            ...
            $data['users'] = $privilegeservice->getEmployeesForPrivilege($priviledgesmodel);

            Priviledgesmodel :: setPrivilege_Code($priv_code);
            $data['title_priv'] = $privilegeservice->getPriviledgebyid();
            $data['title'] = "Users with privileges";

            $partials = array('content' => 'Priviledges/users_per_priv');
            $this->template->load('template/main_template', $partials, $data);
        } else {
            $this->template->load('template/access_denied_page');
        }
