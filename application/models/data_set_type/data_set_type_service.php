...
class Data_set_type_service extends CI_Model {
    ...
    public function get_all_data_set_types() {


        $this->db->select('*');
        $this->db->from('data_set_type');
        $this->db->order_by("data_set_type.data_set_type_id", "desc");
        $query = $this->db->get();
        return $query->result();
    }
