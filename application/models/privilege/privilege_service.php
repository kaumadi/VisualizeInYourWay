...
class Privilege_service extends CI_Model {
    ...
    function get_privileges_by_master_privilege_assigned_for($id, $user) {
        ...
        $this->db->where('privilege.assign_for', $this->config->item('ALL'));
...
//        echo $this->db->last_query();die;
    ...
    function get_privileges_by_master_privilege_assigned_for_user($id, $user, $system) {
    
        $this->db->select('*');
        $this->db->from('privilege');
        $this->db->join('privilege_master', 'privilege_master.privilege_master_code = privilege.privilege_master_code');
        $this->db->where('privilege.privilege_master_code =' . $id);
        $this->db->where('privilege_master.system_code =' . $system);
